...
// its Pulp/Fiber wasmimport capabilities); main.go delegates so the gate has a
// single source of truth that can be unit-tested natively. See
// execallow/execallow.go and execallow/execallow_test.go.
func execCommandAllowed(cmd []string) bool {
	return execallow.Allowed(cmd)
}
